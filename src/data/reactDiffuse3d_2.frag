...
void main(void){
	...
	float w	= texOffset.s;     // horizontal distance between texels - 1/texture width == 1/(width * depth)
	float h	= texOffset.t;     // vertical distance between texels 1/texture height
	float d	= distZ;			//depth dist - next layer is 1/widthX away
...
//	offset3D[0] = vec2(-h, -wd);	offset3D[3] = vec2(-h, -d);		offset3D[6] = vec2(-h, wmd);
//	offset3D[1] = vec2(0.0, -wd);	offset3D[4] = vec2(0.0,-d);		offset3D[7] = vec2(0.0,wmd);
//	offset3D[2] = vec2(h, -wd);		offset3D[5] = vec2(h,  -d);		offset3D[8] = vec2(h,  wmd);
//
//	offset3D[9] = vec2( -h, -w);	offset3D[12] = vec2(-h, 0.0);	offset3D[15] = vec2(-h, w);
//	offset3D[10] = vec2(0.0,-w);	offset3D[13] = vec2(0.0,0.0);	offset3D[16] = vec2(0.0,w);
//	offset3D[11] = vec2(h,  -w);	offset3D[14] = vec2(h, 0.0);	offset3D[17] = vec2(h, w);
//
//	offset3D[18] = vec2(-h, -wmd);	offset3D[21] = vec2(-h, d);		offset3D[24] = vec2(-h, wd);
//	offset3D[19] = vec2(0.0,-wmd);	offset3D[22] = vec2(0.0,d);		offset3D[25] = vec2(0.0,wd);
//	offset3D[20] = vec2(h,  -wmd);	offset3D[23] = vec2(h,  d);		offset3D[26] = vec2(h,  wd);
   ...
   // Loop through the neighbouring pixels and compute Laplacian
	...
	fragColor = vec4( clamp( u, 0.0, 1.0 ), clamp( v, 0.0, 1.0 ), 0, 1.0 );
}
