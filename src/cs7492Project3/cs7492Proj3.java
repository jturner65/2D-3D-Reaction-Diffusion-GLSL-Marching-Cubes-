...
//		      addInfoStr(0,"Click the light green box to the left to toggle showing this message.");
//		      addInfoStr(1,"--Shift-Click-Drag to change view.  Shift-RClick-Drag to zoom.");
//		      addInfoStr(2,"NOTE : Torroidal bounds only work for Forward Euler Simulation.  All others have Neumann boundaries");
//              addInfoStr(3,"Values at Mouse Location : "+RD.getUVKFAtCellAsString());
//			}
//			String[] res = consoleStrings.toArray(new String[0]);
//			int dispNum = min(res.length, 80);
//			for(int i=0;i<dispNum;++i){addInfoStr(res[i]);}
...
//			if(flags[show3D]){drawProjOnBox(dfCtr, new int[] {gui_Red, gui_Red, gui_Green, gui_Green, gui_Blue, gui_Blue});}
//			drawAxes(10000,1f, myPoint.ZEROPT, 100, true);//
//			//draw intercept with box
//			stroke(0,0,0,255);
//			show(myPoint.ZEROPT,3);
//			drawText(""+dfCtr,4, 15, 4,0);
//			scale(1.5f,1.5f,1.5f);
//			drawText(""+getUVKF(dfCtr),4, -8, 4,0);//getUVKF
//		popStyle();
//		popMatrix();		
//	}//drawMseEdge	
//
...
////		myVector planeTan = U(myVector._cross(drawSNorm, U((float)drawSNorm.x+10,(float)drawSNorm.y+10,(float)drawSNorm.z+10)));			//result of vector crossed with normal will be in plane described by normal
...
////     	mseIn3DBox = new myPoint(dfCtr.x+gridDimX/2.0f,dfCtr.y+gridDimY/2.0f,dfCtr.z+gridDimZ/2.0f);
////     	drawMseEdge();
////	}//buildCanvas()
...
//	public myPoint getPlInterSect(myPoint p){
...
//		intersectPl(p, eyeToMse, canvas3D[0],canvas3D[1],canvas3D[2],  dctr);//find point where mouse ray intersects canvas
//		return dctr;		
//	}//getPlInterSect	
//	
////	//line bounded by verts - from a to b new myPoint(x,y,z); 
////	private class edge{ public myPoint a, b;
...
//	public void gl_vertex(myPoint P) {vertex((float)P.x,(float)P.y,(float)P.z);}                                           // vertex for shading or drawing
//	
//	public void drawMyTri(myMCTri t){
//		noStroke();
//        setColorValFill(triColors[RD.dispChem][currDispType]);
